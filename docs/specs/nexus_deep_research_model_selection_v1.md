...
Nexus counterpart: `docs/specs/nexus_hermes_deep_research_connector_handoff_v1_1.md`

## Summary

The Deep Research page's disabled "Managed by Nexus" model field is replaced
with a real searchable model selector. Nexus displays a live, filtered catalog
and sends the chosen model on the task envelope; Nexus independently validates
...
The old local Nexus UI had a Hermes chat model selector, but that direct
UI→Hermes chat architecture is obsolete under the current Nexus→queue→Connector
→Nexus routing. What remains reusable is Nexus's server-side catalog
...
writes `config/model_options.yaml` (the allowlist Nexus validates against).
...
proxies arbitrary URLs, submits research, calls Nexus, or returns the
...
Nexus re-validates independently, so this filter is convenience, not
authority.

## Selector UI

`components/workspace/port/ResearchModelSelector.tsx`: a search box + select.
First option is always **Nexus default** (sends no model id). Concrete options
...
still works with the Nexus default (and any last valid selection). A saved
...
concrete model is chosen (omission = Nexus default). `submitDeepResearch`
...
- No direct Nexus→Hermes or new Nexus→Nexus route; the existing queue +
...
restart Nexus. Model selection is fully wired and offline-verified now.
