...
| File | Change |
|------|--------|
| `docs/console_reform/package_17_competing_authority_legacy_file_audit.md` | **New** — this audit |
| `docs/console_reform/legacy_file_classification.json` | **New** — machine-readable summary |
| `tests/test_nexus_final_audit_artifacts.py` | **New** — artifact presence checks |
...
grep -R "is_console_mode\|block_local_execution\|block_authority\|block_connector" routes src
...
**Internals:** `src/agent_loop.py`, `task_scheduler.py`, `tool_execution.py`, `tool_implementations.py`, `agent_tools.py`, `mcp_manager.py`, `deep_research.py`, `memory.py`, `bg_monitor.py`, guard modules, `nexus_*` bridge/client modules.

**Routes (47 modules):** focus on `task_routes`, `assistant_routes`, `webhook_routes`, `document_routes`, `gallery_routes`, `compare_routes`, `cookbook_routes`, `model_routes`, `chat_routes`, `shell/mcp/research/email/calendar/memory/skills`, `nexus_routes`, `upload_routes`, `personal_routes`, `note_routes`.
...
**UI/static:** `static/login.html`, `index.html`, `landing.html`, `manifest.json`, `sw.js`, `app.js`, `nexusConsoleMode.js`, `nexusDashboard.js`, major `static/js/*` modules (per P15 matrix).
...
| `routes/model_routes.py` | Endpoint CRUD, probe, tools registry | **None** (intentional) | Admin can add endpoints; probes call external APIs | **preserve_for_ollama_or_local_model_status** — document as Nexus admin; optional read-only mode later. |
...
| `routes/nexus_routes.py` | Gateway forward-only | — | keep_active_nexus |
...
| File/path | Current purpose | Nexus role | Classification | Recommendation |
|-----------|-----------------|--------------|----------------|----------------|
| `start-macos.sh` | Native macOS launcher | Primary Nexus Mac start path | **keep_active_nexus** | Keep; add comment pointer to private deployment doc (P18 optional). |
| `launch-windows.ps1` | Windows launcher | Legacy/non-Nexus-Mac dev | **keep_compatibility** | Do not remove until P18; document as non-primary. |
| `Dockerfile` | Container image build | Optional deployment | **keep_compatibility** | P18: label legacy local console env vars in comments only. |
| `docker-compose.yml` | Multi-service stack | Optional deployment | **keep_compatibility** | P18: archive or `docker/README` Nexus notes. |
...
| `build-macos-app.sh` | macOS app bundle | Optional | **keep_compatibility** | needs_review for Nexus branding |
| `install-service.sh` | systemd install | Linux server deploy | **archive_candidate** | P18: move to `scripts/legacy/` or document unused on Nexus Mac. |
...
| `companion/` | LAN mobile pairing API | Private mobile access | **keep_active_nexus** | Align docs with Tailscale/private LAN (P16). |
...
| `docs/console_reform/` | Reform packages | Active program docs | **keep_active_nexus** | — |
| `static/landing.html` | Marketing/ satire page | Served legacy page | **needs_review** | P18: satire still says Odysseus; hero says Nexus. |
| `README.md` | Upstream Odysseus README | Confusing hybrid branding | **needs_review** | P18: Nexus section + link to reform docs (no full rewrite required). |
| `SECURITY.md` | Security policy | Active | **keep_active_nexus** | Updated P16 |
| `THREAT_MODEL.md` | Threat model | Reference | **needs_review** | P18: Nexus-specific threat addendum |
...
| `requirements*.txt` | Python deps | Active | **keep_active_nexus** | — |
...
| Page/static asset | Served/reachable? | Visible brand/status | Nexus role | Recommendation |
|-------------------|-------------------|----------------------|--------------|----------------|
| `static/login.html` | Yes (`/login`) | Nexus (P14) | Console entry | **keep_active_nexus** |
| `static/index.html` | Yes (`/`) | legacy local console title (P14) | Main app shell | **keep_active_nexus** |
| `static/landing.html` | Yes (`/static/landing.html`) | Mixed: Nexus hero, Odysseus satire/history | Marketing/legacy | **needs_review** — P18 copy pass or archive |
| `static/manifest.json` | Yes (PWA) | Nexus (P14) | PWA install | **keep_active_nexus** |
| `static/sw.js` | Yes | Comment/cache `odysseus-v326` | PWA offline shell | **needs_review** — bump cache name in P18 optional |
| `static/app.js` | Yes | Nexus + `startOdysseusApp` | Boot + Console Mode init | **keep_active_nexus** |
| `static/js/nexusConsoleMode.js` | Yes | Nexus | UI gating (P15) | **keep_active_nexus** |
| `static/js/nexusDashboard.js` | Yes | Nexus | Dashboard (P9/P16) | **keep_active_nexus** |
| `static/js/chat.js`, `chatStream.js` | Yes | Nexus UI | Chat (bridge when Console Mode) | **keep_active_nexus** |
| `static/js/cookbook.js` | Yes | Nexus tool | Cookbook UI visible | **needs_review** — P19 backend guards |
...
| `companion` pairing HTML | Yes (`/api/companion/*`) | Nexus routes title | Mobile pairing | **keep_active_nexus** — private LAN only |
...
| Python modules (`src/*`, `routes/*`, `core/*`) | Imports everywhere | **Do not rename** | Nexus is product name; codebase names are compatibility. |
| DB table/model names (`ScheduledTask`, etc.) | SQLAlchemy | **Do not rename** | Requires migrations. |
| FastAPI title "legacy local console" | `app.py` | User-visible only | Already Nexus (P14). |
...
7. **Linux systemd artifacts** — `install-service.sh`, `odysseus-ui.service` on Nexus Mac path (**P18** archive).
8. **`scripts/odysseus-*` CLI** — classify and archive unused (**P18**).

## High-priority keep/compatibility candidates

1. `start-macos.sh` — Nexus Mac primary launcher.
2. `routes/nexus_routes.py` + Nexus static dashboard/console mode JS.
3. `src/console_mode.py` + guard modules + `nexus_*` bridges.
...
| Package | Focus |
|---------|--------|
| **Package 18 — Controlled legacy file cleanup and archive pass** | No behavior change to guards; move/archive `install-service.sh`, unused scripts, duplicate `docs/index.html`; README/THREAT_MODEL Nexus pointers; optional `sw.js` cache bump; do **not** delete Ollama/Docker/Windows yet without explicit operator sign-off. |
...
| **Package 20 — Final safety audit and operator handoff** | End-to-end test matrix, operator runbook, confirm P1–19, deployment checklist on Nexus Mac. |
...
- Focused Nexus tests (P1–P17): **165 passed**
...
- Classification does not replace penetration testing on a deployed Nexus Mac.
