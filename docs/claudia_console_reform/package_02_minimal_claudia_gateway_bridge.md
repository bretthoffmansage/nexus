...
| Field | Value |
|-------|-------|
| **Package** | Package 2 — Minimal Nexus Gateway bridge |
...
| File | Change |
|------|--------|
| `src/nexus_client.py` | **New** — env config, httpx forward/probe, response envelope |
| `routes/nexus_routes.py` | **New** — Gateway routes |
...
| `tests/test_nexus_gateway_routes.py` | **New** — health/intake/forwarding/static-import safety tests |
| `docs/console_reform/package_02_minimal_nexus_gateway_bridge.md` | **New** — this note |
...
| `POST /api/nexus/v1/intake` | **Not exempt** — subject to normal `AuthMiddleware` when auth is enabled (same as most `/api/*` routes). Intake does not execute work locally; forwarding is the only side effect when Core is configured. |
...
1. **Package 3** — Nexus machine token scopes for Gateway/intake callers
