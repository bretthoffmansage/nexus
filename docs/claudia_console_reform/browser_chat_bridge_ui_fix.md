# Claudia Console Browser Chat Bridge UI Fix Pass

**Package / pass name:** Claudia Console Browser Chat Bridge UI Fix Pass  
**Date / time:** 2026-06-03 (local)  
**Repo path:** `/Users/bretthoffman/Documents/claudia_console`

## Summary

Fixed browser Chat mode so Claudia Console routes messages through the existing Gateway → Core → Hermes bridge instead of legacy Odysseus local model/session validation. Preserved Core model selector visibility after send and made legacy warning cards dismissible.

## Files changed

| File | Change |
|------|--------|
| `static/js/claudiaBrowserChatBridge.js` | **New** — detects Core model config, extracts bridge response text |
| `static/js/claudiaConsoleMode.js` | Init bridge module at startup; expose on `window` |
| `static/js/sessions.js` | `ensureClaudiaBridgeSession()` — minimal session without local endpoint |
| `static/js/chat.js` | Bridge path before legacy session warning; SSE `claudia_message` parsing; model picker refresh in `finally` |
| `static/js/chatRenderer.js` | Dismissible warning UI (`msg-dismiss-btn`); Claudia role for bridge warnings |
| `static/app.js` | Skip model-picker autohide when bridge/Core selector active |
| `static/style.css` | `.msg-dismiss-btn` styles |
| `tests/test_claudia_browser_chat_bridge_ui.py` | **New** — static frontend bridge checks |
| `docs/claudia_console_reform/browser_chat_bridge_ui_fix.md` | This note |

**Intentionally unchanged:** `routes/claudia_routes.py`, `routes/chat_routes.py`, `src/claudia_chat_bridge.py` (backend bridge already worked via curl).
...
- When `GET /api/claudia/v1/model-config` reports `core_configured: true`, Chat submit creates/uses a minimal bridge session and sends via existing `POST /api/chat_stream` (backend `console_mode_chat_stream` → Core).
- Legacy “No chat session active / pick a model endpoint” is **not shown** on the bridge path.
- Core-unavailable errors show Claudia-specific messages with `{ dismissible: true }`.
- SSE events `claudia_message`, `error`, and `validation_error` render via `extractAssistantContent()`.
...
**Chosen path:** Option B — continue using `POST /api/chat_stream`, which already routes to `console_mode_chat_stream` when `CLAUDIA_CONSOLE_MODE=true`. Smallest safe fix: fix frontend gating only.

Flow: `handleSubmit` → `ensureClaudiaBridgeSession()` → `/api/chat_stream` → Gateway `forward_message` → Core/Hermes → SSE `type: claudia_message` with `delta`.

## Model selector persistence behavior

- `_syncModelPickerAutohide` skips hide when `window.claudiaBrowserChatBridge.shouldUseBridge()` is true.
- Stream `finally` calls `_syncModelPickerAutohide()` and `updateModelPicker()` so label/state restore after send.
- Selector continues to load/switch via `GET/POST /api/claudia/v1/model-config` (`claudiaModelSelector.js` / `modelPicker.js`).
...
- **Core unavailable:** Claudia-specific error text, dismissible, not Odysseus setup instructions.
...
- `shouldUseBridge()` keys off `core_configured` from model-config, not only `claudia_console_mode` flag — intentional so Core routing works whenever Gateway has Core URL.
- Agent mode toggle still visible; Console backend demotion prevents local agent loop but UX may still show agent chrome.

## Follow-ups

- Optional: direct `POST /api/claudia/v1/messages` from frontend for non-streaming one-shot UI (if SSE overhead is unwanted).
...
3. Confirm chat-bar model selector shows Core active model from `/api/claudia/v1/model-config`.
4. Select **Chat** mode; send a message.
5. Confirm Hermes/Core response renders (assistant bubble, role “Claudia”).
...
| Console Mode + Core unavailable | Legacy local-model warning or hang | Claudia-specific error (dismissible) | None (blocked at session gate) | No |
...
|-------|--------|-------|-------|
| Initial page load | Core selector when wired | Unchanged | `claudiaModelSelector` + `modelPicker` |
...
| Core unavailable | Odysseus-style hint | “Claudia Core is unavailable…” | Yes |
