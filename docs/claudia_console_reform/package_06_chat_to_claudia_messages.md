...
| Field | Value |
|-------|-------|
| **Package** | Package 6 — Chat path demotion, phase 2: Claudia message packets and SSE relay |
...
When `CLAUDIA_CONSOLE_MODE=true`, route browser and API chat submissions through Claudia Gateway message packets (type `message`) and forward to Claudia Core when configured. Emit frontend-safe SSE/JSON with packet/trace metadata. Never invoke Odysseus `stream_agent_loop`, `llm_call_async`, local models, shell, MCP, file tools, email/calendar, memory/skills writes, research, or task scheduler.

## Files changed

| File | Change |
|------|--------|
| `src/claudia_packets.py` | `create_chat_message_packet()` — chat route, session `source_id`, `reply_channel` object |
| `src/claudia_client.py` | `forward_message()` → Core `POST /messages` with 404 fallback to `/intake`; `stream_packet_events()` placeholder SSE |
| `src/claudia_chat_bridge.py` | **New** — Console Mode chat stream/sync/resume → message packet + SSE/JSON |
| `routes/claudia_routes.py` | `POST /messages`, `GET /stream/{packet_id}` |
...
| `tests/test_claudia_messages.py` | **New** — message packet, routes, stream SSE, console bridge |
| `tests/test_claudia_chat_demotion.py` | Updated — expects `console_mode_chat_stream` before `stream_agent_loop` |
| `tests/test_claudia_gateway_routes.py` | Assertion text aligned with Core-unreachable message |
| `docs/claudia_console_reform/package_06_chat_to_claudia_messages.md` | **New** — this note |
...
|--------|------|------|---------|
| `POST` | `/api/claudia/v1/messages` | `claudia_intake` (Bearer) or session when auth enabled | Normalize `type=message` packet; forward to Core |
| `GET` | `/api/claudia/v1/stream/{packet_id}` | `claudia_read` (Bearer) or session | Bounded placeholder SSE for packet status |

## Behavior changed

### Claudia Console Mode (`CLAUDIA_CONSOLE_MODE=true`)

- **`POST /api/chat_stream`** — builds `create_chat_message_packet()`, calls `forward_message()`, returns SSE via `sse_from_gateway_result()` (`type: claudia_message`, `delta`, `packet_id`, `trace_id`, `[DONE]`). No `stream_agent_loop`.
...
- **`POST /api/claudia/v1/messages`** — accepts JSON; normalizes or builds chat message packet; forwards via `forward_message()`.
- **`GET /api/claudia/v1/stream/{packet_id}`** — placeholder SSE until Core event stream exists.
...
- No `/api/claudia/v1/sources`, worker-output, approvals, dashboard UI, packet persistence.
...
```
data: {"type":"claudia_message","message":"...","delta":"...","packet_id":"...","trace_id":"...","ok":...,"status":"...","core_configured":...,"forwarded":...,"claudia_console_mode":true,"agent_disabled":true}

data: [DONE]
```

**Gateway stream (`GET /api/claudia/v1/stream/{packet_id}`):**
...
`CLAUDIA_CORE_URL` unset → `forward_message()` returns `ok: false`, `status: core_not_configured`, `forwarded: false`. Chat SSE/JSON explains Core is not configured and **local agent did not run**.
...
|-------|--------------|------------------|
| `POST /api/claudia/v1/messages` | Requires `claudia_intake` when auth enabled | Allowed via `authorize_claudia_intake` |
| `GET /api/claudia/v1/stream/{packet_id}` | Requires `claudia_read` when auth enabled | Allowed via `authorize_claudia_read` |
...
|----------|----------------------|---------------------------------------|------------------------------------------|
| **POST /api/chat_stream** | Agent or chat SSE via `stream_agent_loop` / `stream_llm` | Message packet → `forward_message()` → Claudia SSE | **No** |
...
| **POST /api/claudia/v1/messages** | N/A (Gateway) | Normalize + forward to Core `/messages` (404 → `/intake`) | **No** |
| **GET /api/claudia/v1/stream/{packet_id}** | N/A (Gateway) | Placeholder SSE (packet status) | **No** |

## `/api/chat` and `/api/v1/chat` decisions

- **`POST /api/chat`** — routed to Claudia message packet (low risk; same bridge as stream).
- **`POST /api/v1/chat`** — routed to `console_mode_sync_chat()` (replaces Package 5 static JSON only).

## Safety guarantees

1. Console Mode chat is Claudia-bound, not Odysseus-agent-bound.
...
6. Legacy mode unchanged when `CLAUDIA_CONSOLE_MODE=false`.
...
- Proxy Core `GET /events` (or equivalent) when Claudia Core exposes packet streams.
