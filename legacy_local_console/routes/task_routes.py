...
def setup_task_routes(task_scheduler) -> APIRouter:
    def _task_exec_blocked(operation: str):
        from src.execution_console_guard import block_local_execution

        return block_local_execution("tasks", operation)
    ...
    async def run_task_now(request: Request, task_id: str, force: bool = False):
        if blocked := _task_exec_blocked("run"):
            return blocked
        user = _owner(request)
        db = SessionLocal()
        try:
    ...
    async def webhook_trigger(task_id: str, token: str):
        ...
        if blocked := _task_exec_blocked("webhook_trigger"):
            return blocked
        db = SessionLocal()
        try:
