...
def setup_mcp_routes(mcp_manager: McpManager):
    ...
    def _mcp_exec_blocked(operation: str):
        from src.execution_console_guard import block_local_execution

        return block_local_execution("mcp", operation)
    ...
    async def add_server(
        ...
        require_admin(request)
        if blocked := _mcp_exec_blocked("add_server"):
            return blocked
    ...
    async def reconnect_server(server_id: str, request: Request):
        ...
        require_admin(request)
        if blocked := _mcp_exec_blocked("reconnect"):
            return blocked
        db = SessionLocal()
        try:
    ...
    async def toggle_server(server_id: str, request: Request, is_enabled: str = Form(...)):
        ...
        if enabled:
            if blocked := _mcp_exec_blocked("connect"):
                return blocked
        db = SessionLocal()
        try:
            srv = db.query(McpServer).filter(McpServer.id == server_id).first()
    ...
    async def _exchange_and_connect(server_id: str, code: str, request: Request):
        ...
        if blocked := _mcp_exec_blocked("oauth_connect"):
            return HTMLResponse(
                f"<html><body><h1>Blocked</h1><p>{blocked['message']}</p></body></html>",
                status_code=403,
            )
        db = SessionLocal()
        try:
