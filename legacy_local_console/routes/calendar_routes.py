...
def setup_calendar_routes() -> APIRouter:
    def _calendar_write_blocked(operation: str):
        from src.connector_console_guard import block_connector_write

        return block_connector_write("calendar", operation)
    ...
    async def create_event(request: Request, data: EventCreate):
        if blocked := _calendar_write_blocked("create_event"):
            return blocked
        owner = _require_user(request)
        db = SessionLocal()
        try:
    ...
    async def update_event(request: Request, uid: str, data: EventUpdate):
        if blocked := _calendar_write_blocked("update_event"):
            return blocked
        owner = _require_user(request)
        try:
    ...
    async def delete_event(request: Request, uid: str):
        if blocked := _calendar_write_blocked("delete_event"):
            return blocked
        owner = _require_user(request)
        try:
            base_uid = _resolve_base_uid(uid)
