...
def setup_gallery_routes() -> APIRouter:
    ...
    def _gallery_generation_blocked(operation: str):
        from src.authority_console_guard import block_authority

        return block_authority("gallery", operation)
    ...
    async def gallery_ai_upscale(request: Request):
        ...
        if blocked := _gallery_generation_blocked("ai_upscale"):
            return blocked
        import base64, httpx
    ...
    async def gallery_style_transfer(request: Request):
        ...
        if blocked := _gallery_generation_blocked("style_transfer"):
            return blocked
        import base64, httpx
    ...
    async def ai_tag_batch(
        ...
        if blocked := _gallery_generation_blocked("ai_tag_batch"):
            return blocked
        user = get_current_user(request)
        db = SessionLocal()
        try:
    ...
    async def inpaint_proxy(request: Request):
        ...
        if blocked := _gallery_generation_blocked("inpaint"):
            return blocked
        import httpx
        require_privilege(request, "can_generate_images")
    ...
    async def harmonize_image(request: Request):
        ...
        if blocked := _gallery_generation_blocked("harmonize"):
            return blocked
    ...
    async def sharpen_image(request: Request):
        ...
        if blocked := _gallery_generation_blocked("sharpen"):
            return blocked
        body = await request.json()
        image_b64 = body.get("image")
    ...
    async def denoise_image(request: Request):
        if blocked := _gallery_generation_blocked("denoise"):
            return blocked
        require_privilege(request, "can_generate_images")
        body = await request.json()
        image_b64 = body.get("image")
    ...
    async def upscale_image_local(request: Request):
        if blocked := _gallery_generation_blocked("upscale_local"):
            return blocked
        require_privilege(request, "can_generate_images")
        body = await request.json()
        image_b64 = body.get("image")
    ...
    async def remove_background(request: Request):
        ...
        image), we constrain the output: crop to mask bbox, run rembg,
        paste back, multiply alpha by hint mask.
        """
        if blocked := _gallery_generation_blocked("remove_bg"):
            return blocked
        require_privilege(request, "can_generate_images")
        body = await request.json()
        image_b64 = body.get("image")
    ...
    async def enhance_face(request: Request):
        ...
        if blocked := _gallery_generation_blocked("enhance_face"):
            return blocked
        require_privilege(request, "can_generate_images")
        body = await request.json()
        image_b64 = body.get("image")
    ...
    async def ai_tag_image(request: Request, image_id: str):
        ...
        if blocked := _gallery_generation_blocked("ai_tag"):
            return blocked
        import base64, httpx
        from pathlib import Path
