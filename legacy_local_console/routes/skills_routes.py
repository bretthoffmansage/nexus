...
def setup_skills_routes(skills_manager: SkillsManager) -> APIRouter:
    ...
    def _skills_authority_blocked(operation: str):
        from src.authority_console_guard import block_authority

        return block_authority("skills", operation)
    ...
    async def set_builtin_override(name: str, request: Request):
        ...
        if blocked := _skills_authority_blocked("builtin_override"):
            return blocked
    ...
    async def reset_builtin_override(name: str, request: Request):
        ...
        if blocked := _skills_authority_blocked("builtin_reset"):
            return blocked
        require_admin(request)
    ...
    async def add_skill(request: Request, body: SkillAddRequest):
        if blocked := _skills_authority_blocked("add"):
            return blocked
        user = _owner(request)
    ...
    async def test_skill(request: Request, skill_id: str):
        ...
        if blocked := _skills_authority_blocked("test"):
            return blocked
        import time as _time
        import asyncio as _asyncio
        from src.endpoint_resolver import resolve_endpoint
    ...
    async def audit_all_skills(request: Request):
        ...
        if blocked := _skills_authority_blocked("audit_all"):
            return blocked
        import asyncio as _asyncio
        import time as _time
    ...
    async def save_skill_markdown(request: Request, skill_id: str):
        ...
        if blocked := _skills_authority_blocked("save_markdown"):
            return blocked
    ...
    async def update_skill(request: Request, skill_id: str, body: SkillUpdateRequest):
        if blocked := _skills_authority_blocked("update"):
            return blocked
        user = _owner(request)
        skills = skills_manager.load(owner=user)
        match = next((s for s in skills if s.get("name") == skill_id or s.get("id") == skill_id), None)
    ...
    async def delete_skill(request: Request, skill_id: str):
        if blocked := _skills_authority_blocked("delete"):
            return blocked
        user = _owner(request)
        skills = skills_manager.load(owner=user)
        match = next((s for s in skills if s.get("name") == skill_id or s.get("id") == skill_id), None)
