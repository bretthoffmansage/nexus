...
def setup_document_routes(session_manager, upload_handler=None) -> APIRouter:
    ...
    def _file_write_blocked(operation: str):
        from src.execution_console_guard import block_local_execution

        return block_local_execution("file", operation)
    ...
    async def create_document(request: Request, req: DocumentCreate) -> Dict[str, Any]:
        if blocked := _file_write_blocked("create"):
            return blocked
    ...
    async def import_pdf(
        ...
        overlays.
        """
        if blocked := _file_write_blocked("import_pdf"):
            return blocked
    ...
    async def archive_document(request: Request, doc_id: str, archived: bool = Query(True)) -> Dict[str, Any]:
        if blocked := _file_write_blocked("archive"):
            return blocked
        user = get_current_user(request)
        db = SessionLocal()
        try:
    ...
    async def update_document(request: Request, doc_id: str, req: DocumentUpdate) -> Dict[str, Any]:
        if blocked := _file_write_blocked("update"):
            return blocked
        user = get_current_user(request)
        db = SessionLocal()
        try:
    ...
    async def delete_document(request: Request, doc_id: str) -> Dict[str, str]:
        if blocked := _file_write_blocked("delete"):
            return blocked
        user = get_current_user(request)
        db = SessionLocal()
        try:
    ...
    async def restore_version(request: Request, doc_id: str, num: int) -> Dict[str, Any]:
        if blocked := _file_write_blocked("restore"):
            return blocked
        user = get_current_user(request)
        db = SessionLocal()
        try:
    ...
    async def ai_tidy_documents(request: Request) -> Dict[str, Any]:
        ...
        if blocked := _llm_assist_blocked("ai_tidy"):
            return blocked
    ...
    async def ai_fill_annotations(doc_id: str, request: Request) -> Dict[str, Any]:
        ...
        """
        if blocked := _llm_assist_blocked("ai_fill_annotations"):
            return blocked
        import base64
        import json
        import fitz
