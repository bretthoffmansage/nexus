...
def setup_memory_routes(memory_manager: MemoryManager, session_manager: SessionManager, memory_vector=None):
    ...
    def _memory_authority_blocked(operation: str):
        from src.authority_console_guard import block_authority

        return block_authority("memory", operation)
    ...
    async def api_add_memory(
        ...
        if blocked := _memory_authority_blocked("add"):
            return blocked
    ...
    async def extract_memory(request: Request, session: str = Form(...)) -> Dict[str, List[str]]:
        ...
        if blocked := _memory_authority_blocked("extract"):
            return blocked
    ...
    async def api_audit_memories(request: Request, session: str = Form(None)):
        ...
        """
        if blocked := _memory_authority_blocked("audit"):
            return blocked
    ...
    async def import_memories_from_file(
        ...
        if blocked := _memory_authority_blocked("import"):
            return blocked
        from src.auth_helpers import require_privilege
        require_privilege(request, "can_manage_memory")
    ...
    def pin_memory(request: Request, memory_id: str, pinned: bool = Form(True)):
        ...
        if blocked := _memory_authority_blocked("pin"):
            return blocked
        user = _owner(request)
        all_mem = memory_manager.load_all()
    ...
    def update_memory(request: Request, memory_id: str, text: str = Form(...), category: str = Form(None)):
        ...
        if blocked := _memory_authority_blocked("update"):
            return blocked
        user = _owner(request)
        all_mem = memory_manager.load_all()
        for i, memory in enumerate(all_mem):
    ...
    def delete_memory(request: Request, memory_id: str):
        ...
        if blocked := _memory_authority_blocked("delete"):
            return blocked
        user = _owner(request)
        all_mem = memory_manager.load_all()
