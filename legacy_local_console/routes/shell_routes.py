...
def setup_shell_routes() -> APIRouter:
    ...
    def _shell_exec_blocked(operation: str):
        from src.execution_console_guard import block_local_execution

        return block_local_execution("shell", operation)
    ...
    async def shell_exec(request: Request, req: ShellExecRequest) -> Dict[str, Any]:
        ...
        if blocked := _shell_exec_blocked("exec"):
            return blocked
    ...
    async def shell_stream(request: Request, req: ShellExecRequest):
        ...
        _require_admin(request)
        if blocked := _shell_exec_blocked("stream"):
            from src.execution_console_guard import local_execution_disabled_sse

            return StreamingResponse(
                local_execution_disabled_sse("shell", "stream"),
    ...
    async def install_package(request: Request):
        ...
        _require_admin(request)
        if blocked := _shell_exec_blocked("cookbook_install"):
            return blocked
        import sys as _sys
