...
def setup_cookbook_routes() -> APIRouter:
    ...
    def _cookbook_exec_blocked(operation: str):
        from src.execution_console_guard import block_local_execution

        return block_local_execution("cookbook", operation)
    ...
    async def generate_cookbook_ssh_key(request: Request):
        if blocked := _cookbook_exec_blocked("ssh_key_generate"):
            return blocked
        require_admin(request)
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        if blocked := _cookbook_exec_blocked("download"):
            return blocked
    ...
    async def model_serve(request: Request, req: ServeRequest):
        if blocked := _cookbook_exec_blocked("serve"):
            return blocked
    ...
    async def server_setup(request: Request, req: SetupRequest):
        ...
        if blocked := _cookbook_exec_blocked("setup"):
            return blocked
        require_admin(request)
    ...
    async def kill_pid(request: Request, req: KillPidRequest):
        ...
        """
        if blocked := _cookbook_exec_blocked("kill_pid"):
            return blocked
        require_admin(request)
        if req.pid < 100:
