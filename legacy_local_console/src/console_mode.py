"""legacy local console Mode — env-driven startup gates for in-process autonomy.

When enabled, Odysseus serves UI/API without starting background systems that
compete with Nexus Core (task runner, email pollers, agent auto-continuation).
"""
...
_ENV_CONSOLE = "NEXUS_CONSOLE_MODE"
...
# Values that mean "on" for NEXUS_CONSOLE_MODE (explicit enable).
_CONSOLE_ON = frozenset({"1", "true", "yes", "on"})


def is_console_mode() -> bool:
    """True when Odysseus runs as legacy local console UI/API shell only."""
