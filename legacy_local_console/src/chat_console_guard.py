"""legacy local console Mode guards for legacy chat endpoints (Package 5).

Blocks local Odysseus chat/agent execution when ``NEXUS_CONSOLE_MODE`` is on.
Does not call Nexus Core, local models, or ``stream_agent_loop``.
"""
...
CONSOLE_MODE_CHAT_MESSAGE = (
    "legacy local console Mode is active. Local Odysseus chat execution is disabled. "
    "Nexus Core message routing is not enabled yet."
)
