    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Claudia';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted || metadata?.claudia_bridge) ? 'Claudia' : shortModel(resolvedModel);
    ...
    if (metadata?.dismissible) {
      wrap.classList.add('msg-dismissible');
    ...
    if (role === 'assistant') {
      // Informational bridge/legacy warnings without db id — skip regen/edit footer noise.
      if (metadata?.dismissible && !metadata?._db_id) {
        // Dismiss control only; no action footer.
      } else {
