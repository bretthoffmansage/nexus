...
function _initModelPickerDropdown() {
  ...
  function _populateCoreModels(filter) {
    ...
    if (unavailable && !hasModels) {
      const empty = document.createElement('div');
      empty.className = 'model-switch-empty mp-core-unavailable';
    ...
    models.forEach((m) => {
      ...
      row.className = 'model-switch-item' + (m.current ? ' mp-core-current' : '');
      const _mlogo = providerLogo(m.id);
      if (_mlogo) {
      ...
      nameSpan.textContent = m.label || m.id;
      row.appendChild(nameSpan);
      if (m.id && m.label && m.label !== m.id) {
        const epSpan = document.createElement('span');
        epSpan.className = 'model-switch-ep';
        epSpan.textContent = m.id;
  ...
  async function _pickCoreModel(m) {
    if (!m || !m.id) return;
    if (m.current) {
      _close();
      return;
    ...
    if (result.ok) {
      ...
      uiModule.showError(result.message || 'Failed to switch model');
    }
  }
  ...
  btn.addEventListener('click', (e) => {
    ...
    if (menu.classList.contains('hidden') || menu.classList.contains('closing')) {
      ...
      } else if (window.modelsModule && window.modelsModule.refreshModels) {
