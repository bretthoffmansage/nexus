...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (_metaCountEl && _chatHistEl) {
    ...
    const _updateMsgCount = () => {
      ...
      const n = _chatHistEl.querySelectorAll(':scope > .msg:not(.tool-state-banner)').length;
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  function applyModeToToggles(mode) {
    ...
    syncWebSearchStateBanner(Boolean(loadToolPref('web', mode)));
  }
  ...
  function setupToggle(btnId, checkboxId, stateKey) {
    ...
    btn.addEventListener('click', () => {
      ...
      if (stateKey === 'web') {
        syncWebSearchStateBanner(chk.checked);
      } else if (chk.checked) {
        _showToolSplash(stateKey);
      }
  ...
  syncWebSearchStateBanner(Boolean(el('web-toggle')?.checked));
...
async function startOdysseusApp() {
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', (e) => {
      ...
      // Welcome screen: muted send arrow — focus input instead of no-op submit
      if (!hasText && !hasFiles && sendBtn.dataset.mode === 'idle') {
        const isWelcome = document.getElementById('chat-container')?.classList.contains('welcome-active');
        if (isWelcome && messageInput) {
          messageInput.focus();
          return;
        }
      }
  ...
  window.syncWebSearchStateBanner = syncWebSearchStateBanner;
