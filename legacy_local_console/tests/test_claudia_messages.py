"""Tests for Claudia Gateway messages and Console Mode chat routing (Package 6)."""
...
def _build_claudia_app(middleware_cls=None):
...
def test_create_chat_message_packet_route_and_session():
    from src.claudia_packets import create_chat_message_packet
...
def test_post_messages_normalizes(monkeypatch):
    ...
    app = _build_claudia_app(_Mw)
    with TestClient(app) as client:
        resp = client.post(
            "/api/claudia/v1/messages",
...
def test_get_stream_returns_sse(monkeypatch):
    ...
    app = _build_claudia_app()
    with TestClient(app) as client:
        resp = client.get("/api/claudia/v1/stream/pkt-abc")
    assert resp.status_code == 200
    assert "text/event-stream" in resp.headers.get("content-type", "")
    assert "claudia_stream_placeholder" in resp.text
...
async def test_console_mode_chat_stream_sse(monkeypatch):
    ...
    from src.claudia_chat_bridge import console_mode_chat_stream
    ...
    assert any("claudia_message" in c for c in chunks)
...
async def test_console_mode_chat_stream_no_agent_loop(monkeypatch):
    ...
    assert payload.get("claudia_console_mode") is True
