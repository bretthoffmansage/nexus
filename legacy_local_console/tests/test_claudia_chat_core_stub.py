...
def _stub_core_body(user_message: str = "hello") -> dict:
    content = (
        f'Claudia Core received your message: "{user_message}". '
    ...
    return {
        ...
        "message": "Claudia Core accepted the message.",
        "response": {
            "role": "assistant",
            "content": content,
            "type": "message_stub",
...
def _forwarded_stub_result(user_message: str = "hello") -> dict:
    ...
    return {
        "ok": True,
        "status": "forwarded",
        "message": "Message forwarded to Claudia Core.",
...
def test_user_visible_falls_back_to_core_message_when_no_content():
    result = _forwarded_stub_result()
    result["core_body"]["response"] = {"role": "assistant", "type": "message_stub"}
    assert _user_visible_from_result(result) == "Claudia Core accepted the message."
...
async def test_console_mode_sync_chat_renders_stub(monkeypatch):
    ...
    monkeypatch.setattr("src.claudia_chat_bridge.forward_message", _fake_forward)
...
async def test_console_mode_chat_no_agent_loop_on_core_stub(monkeypatch):
    ...
    assert payload["claudia_console_mode"] is True
    ...
    mock_resp = type(
        ...
        (),
        {
            ...
            "json": lambda self: {
                "ok": False,
                "error": "unauthorized",
                "message": "Invalid or missing Claudia Gateway secret.",
            },
        },
    )()
    mock_client = AsyncMock()
