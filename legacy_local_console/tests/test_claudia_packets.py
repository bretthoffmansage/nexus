"""Tests for Nexus Gateway packet normalization (Package 4)."""
...
from src.nexus_packets import PacketNormalizeError, normalize_nexus_packet
...
def test_fully_formed_packet_preserved():
    ...
    out = normalize_nexus_packet(body, created_by="ignored")
...
def test_missing_packet_id_and_trace_id_generated():
    out = normalize_nexus_packet({"payload": {"x": 1}}, created_by="alice")
...
def test_intake_validation_error_422(monkeypatch):
    ...
    with TestClient(app) as client:
        resp = client.post("/api/nexus/v1/intake", json={"type": "bogus"})
...
def test_intake_core_unconfigured_returns_normalized_ids(monkeypatch):
    ...
    app = _build_gateway_app()
    with TestClient(app) as client:
        resp = client.post("/api/nexus/v1/intake", json={"payload": {"a": 1}})
    data = resp.json()
...
async def test_forward_sends_normalized_packet(monkeypatch):
    ...
    normalized = normalize_nexus_packet(
        {
            "packet_id": "pkt-n",
            "trace_id": "tr-n",
            "route": "custom:route",
...
def test_packets_module_does_not_import_agent_loop():
    ...
    tree = ast.parse(
        (Path(__file__).resolve().parents[1] / "src/nexus_packets.py").read_text(
            encoding="utf-8"
        )
    )
