"""Tests for legacy local console Mode chat backend demotion (Package 5)."""
...
def test_chat_stream_console_guard_calls_nexus_bridge():
...
def test_guard_precedes_agent_loop_in_chat_stream_source():
    ...
    resume_start = text.index("async def chat_resume", stream_start)
    segment = text[stream_start:resume_start]
    assert "is_console_mode()" in segment
    guard_pos = segment.index("is_console_mode()")
...
def test_guard_precedes_llm_call_in_chat_endpoint_source():
    ...
    assert segment.index("is_console_mode()") < segment.index("llm_call_async(")
...
def test_chat_guard_module_has_no_agent_loop_import():
    ...
    tree = ast.parse((_REPO / "src/nexus_chat_bridge.py").read_text(encoding="utf-8"))
